...
export async function POST(req: NextRequest) {
  try {
    ...
    // ✅ Resend يتعمل داخل الـ handler مش على مستوى الملف
    // بكذا الـ build ما ينكسرش حتى لو الـ key مش موجود
    ...
    const tableRows = rows.map(([k, v]) => `
      <tr>
        <td style="padding:8px 12px;background:#fff;border:1px solid #eee;color:#666;font-size:13px;width:130px;font-weight:600;">${k}</td>
    ...
    const steps = [
      ...
      'ستحصل على رابط تفعيل حسابك',
      'ستبدأ فترتك التجريبية المجانية فوراً',
    ];

    const stepRows = steps.map((s, i) => `
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
        <div style="width:24px;height:24px;background:linear-gradient(135deg,#C9A84C,#E8C97A);border-radius:50%;display:grid;place-items:center;flex-shrink:0;font-size:11px;font-weight:800;color:#0A1628;">${i + 1}</div>
        <span style="color:#FAFAF8;font-size:13.5px;">${s}</span>
      </div>`).join('');

    // ── إيميل تأكيد للعميل ──
    await resend.emails.send({
      from:    'BidLX <no-reply@bidlx.com>',
      to:      email,
...
<body style="margin:0;padding:0;background:#F4F6FA;font-family:'Segoe UI',Tahoma,Arial,sans-serif;">
  <div style="max-width:560px;margin:32px auto;background:#0F2040;border-radius:20px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.3);">
    ...
    <div style="padding:32px 36px;">
      <p style="color:#FAFAF8;font-size:16px;margin:0 0 6px;">مرحباً <strong style="color:#E8C97A;">${name}</strong>،</p>
      <p style="color:#8A9BB5;font-size:14px;line-height:1.7;margin:0 0 24px;">
        شكراً لتسجيلك في BidLX. سيتواصل معك فريقنا خلال
        <strong style="color:#C9A84C;">24 ساعة</strong>
        لتفعيل حسابك وبدء الشهر الأول المجاني.
      </p>
    ...
    // ── إشعار داخلي لـ support@bidlx.com ──
    await resend.emails.send({
